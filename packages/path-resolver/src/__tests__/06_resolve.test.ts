...
describe(`resolve`, () => {
  ...
  test('.', () => {
    ...
    // root not-found
    res = pathResolver.resolve('/some/not-exist') as PathResolveResult
    expect(res).toBeFalsy()

    // '/'
    res = pathResolver.resolve('/') as PathResolveResult
    expect(res).toBeTruthy()
    expect(res.route.note).toEqual({title: 'Index page'})

    // '/books/:year/:genre'
    res = pathResolver.resolve('/books/2020/comics') as PathResolveResult
    expect(res).toBeTruthy()
    ...
    // '/books/(.*)'
    res = pathResolver.resolve('/books/hello/world/123') as PathResolveResult
    expect(res).toBeTruthy()
    expect(res.route.redirectTo).toEqual('/auto')

    // '/team/:id/group/:gr_id'
    res = pathResolver.resolve('/team/0/group/12') as PathResolveResult
    expect(res).toBeTruthy()
    params = res.pathParams
    expect(params).toBeTruthy()
    expect(Object.keys(params).length).toEqual(2)
    ...
    // '/team/:id/user/:name'
    res = pathResolver.resolve('/team/56/user/tom') as PathResolveResult
    expect(res).toBeTruthy()
    params = res.pathParams
    expect(Object.keys(params).length).toEqual(2)
    ...
    // '/team/:id/hr'
    res = pathResolver.resolve('/team/1/hr') as PathResolveResult
    expect(res).toBeTruthy()
    expect(res.route.redirectTo).toEqual('/team/1')

    // '/team/:id/(.*)'
    res = pathResolver.resolve('/team/7/whatwg') as PathResolveResult
    expect(res).toBeTruthy()
    expect(res.route.customTo).toEqual({pathname: '/books/2020/comics'})

    // '/auto'
    res = pathResolver.resolve('/auto') as PathResolveResult
    expect(res).toBeTruthy()
    expect(res.route.component).toEqual('z')
    expect(res.route.note).toEqual({head: 'Auto'})

    // '/auto/:color'
    res = pathResolver.resolve('/auto/yellow') as PathResolveResult
    expect(res).toBeTruthy()
    params = res.pathParams
    ...
    // '/auto/check/redirect'
    res = pathResolver.resolve('/auto/check/redirect') as PathResolveResult
    expect(res).toBeTruthy()
